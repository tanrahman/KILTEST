...
const unsigned char G_ATR[] = {0x0D,0x3B,0x96,0x96,0x80,0x3F,0x47,0xA0,0x81,0x17,0x01,0xDD,0xB0,0x19};
__attribute__((zero_init)) volatile unsigned char G_bATR_Done;
__attribute__((zero_init)) volatile unsigned char G_SCIRecieving;
volatile unsigned short G_SCIRecieveIndex;
volatile unsigned char G_SCIRecieveDone;
volatile unsigned short G_SCISendIndex;
unsigned char G_bSCILeSendDone;
...
unsigned short SW1SW2_SWP;
unsigned short SW1SW2_SWP_APDU_Gate;
...
int main(void)
{
	...
	GVar_SCI_Init();
	RF_Config();
    (*((volatile unsigned long *)(0x50007000 + 0x88 ))) = 0x01;
    G_cM1_Mode = 0x04;
	...
	while(1)
	{
		if(G_bCLKSwitchValid == 0)
		{
			...
			else if(G_bSWP_busy == 1)
			{
      			if(G_bPowerMode == 0)
				{
					HED_ChangeSYSFreq(SYSCLK_AHB_18Mhz,APB_EQU_AHB);
		...
		if(result == 0x11)
		{
			if(G_HCIMode == 0)
			{
				if((result == 0x11) && (G_bRFIndicator == 0))
				{
					openWrSwitch();
                    SWPAPDU_Dispatch();
                    closeWrSwitch();
